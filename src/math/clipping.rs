...
pub fn clip_polyline<F: Scalar>(p: &[Point2<F>], r: &Rect<F>) -> Vec<Vec<Point2<F>>> {
...
///
/// Use Liang-Barksy.
pub fn clip_line<F: Scalar>(a0: &Point2<F>, a1: &Point2<F>, r: &Rect<F>) -> ClipResult<F> {
    let zero = F::zero();
    let one = F::one();
...
mod tests {
    ...
    fn test_outside() {
        let tr = Rect {
            p: [Point2::new(0.0, 0.0), Point2::new(1.0, 2.0)],
        };
